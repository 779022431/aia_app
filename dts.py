...
class App:
    client = None
    dtsDomain = 'dts.aliyuncs.com'
    dtsVersion = '2020-01-01'
    ...
    def DtsDescribeSynchronizationJobStatus(self, jobId):
        action = 'DescribeSynchronizationJobStatus'
        param = {'SynchronizationJobId': jobId}
        request = self.__build_request(self.dtsDomain, self.dtsVersion, action, param)
...
jobId = config.env('dts', 'jobId')
app = App(clientId, clientSecret, region)
ret = app.DtsDescribeSynchronizationJobStatus(jobId)
print ret
